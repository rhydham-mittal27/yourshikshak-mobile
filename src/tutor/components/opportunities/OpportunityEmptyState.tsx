...
const TIPS = [
  "Top tutors check back every morning ðŸŒ…",
  "New leads are posted throughout the day",
  "YourShikshak is expanding Pan India â€” more leads coming! ðŸ‡®ðŸ‡³",
];
...
const OpportunityEmptyState: React.FC<Props> = ({ onRefresh }) => {
  ...
  return (
    <Animated.View style={[s.wrap, { opacity: fadeAnim }]}>
      <Animated.View style={{ transform: [{ translateY: bounceAnim }] }}>
        <View style={s.mascotRing}>
          <LinearGradient
            ...
            style={s.mascotGrad}
          >
            <Text style={s.mascotEmoji}>ðŸ¦‰</Text>
          </LinearGradient>
          <View style={[s.sparkle, { top: 2, right: 6 }]}>
            <Text style={s.sparkleTxt}>âœ¨</Text>
          </View>
          <View style={[s.sparkle, { bottom: 4, left: 4 }]}>
            <Text style={[s.sparkleTxt, { fontSize: 10 }]}>â­</Text>
          </View>
        </View>
      </Animated.View>
      ...
      <Text style={s.foot}>We'll notify you the moment one drops ðŸ””</Text>
    </Animated.View>
  );
};
