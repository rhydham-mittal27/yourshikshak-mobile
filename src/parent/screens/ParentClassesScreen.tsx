/**
 * ParentClassesScreen â€” Calendar + Attendance Verification
...
import {
  ...
  requestReschedule,
  getParentRescheduleHistory,
  ParentSession,
  ParentActiveClass,
  ParentRescheduleHistoryItem,
} from "../../api/client";
...
// â”€â”€â”€ Status dot config â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ Timetable Panel â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
const TimetablePanel = ({ activeClass }: { activeClass: ParentActiveClass }) => {
  ...
  const timeSlot = activeClass.schedule?.timeSlot ?? activeClass.nextSessionTime ?? "â€”";
...
const ParentClassesScreen = ({ name }: { userId: string; name: string; role: string }) => {
  ...
  const [rescheduleHistory, setRescheduleHistory] = useState<ParentRescheduleHistoryItem[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    await loadSessions();
    try {
      const histRes = await getParentRescheduleHistory();
      setRescheduleHistory(histRes.data ?? []);
    } catch { /* silent */ }
    setLoading(false);
  }, [loadSessions]);
        ...
        >
          ...
          {totalCount > 0 && (
            <View style={s.consistencyBadge}>
              ...
              <Text style={s.consistencyTxt}>{consistencyPct}% Consistency Â· {MONTHS[viewDate.getMonth()]} {viewDate.getFullYear()}</Text>
            </View>
          )}
        </LinearGradient>
        ...
        <View style={s.body}>
          ...
          {verificationStatus === "VERIFIED" && (
            <View style={[av.card, av.cardVerified]}>
              ...
              <Text style={av.verifiedTxt}>âœ“ Verified â€” {MONTHS[viewDate.getMonth()]} {viewDate.getFullYear()}</Text>
            </View>
          )}

          {/* â”€â”€ Summary â”€â”€ */}
          {/* â”€â”€ Reschedule History â”€â”€ */}
          {rescheduleHistory.length > 0 && (
            <View style={rh.card}>
              <View style={rh.header}>
                <Ionicons name="swap-horizontal-outline" size={16} color={T.primary} />
                <Text style={rh.headerTxt}>Reschedule Requests</Text>
              </View>
              {rescheduleHistory.slice(0, 5).map((item) => {
                const statusColor = item.status === "APPROVED" ? T.success : item.status === "REJECTED" ? T.error : T.warning;
                const statusIcon = item.status === "APPROVED" ? "checkmark-circle" : item.status === "REJECTED" ? "close-circle" : "time-outline";
                const fmtD = (d: string) => {
                  const dt = new Date(d);
                  return `${MONTHS[dt.getMonth()]} ${dt.getDate()}`;
                };
                return (
                  <View key={String(item.requestId)} style={rh.row}>
                    <View style={[rh.statusDot, { backgroundColor: `${statusColor}15` }]}>
                      <Ionicons name={statusIcon as any} size={14} color={statusColor} />
                    </View>
                    <View style={{ flex: 1 }}>
                      <Text style={rh.rowTitle}>
                        {fmtD(item.fromDate)} â†’ {fmtD(item.toDate)}
                        {item.subject ? <Text style={rh.rowSub}> Â· {item.subject}</Text> : null}
                      </Text>
                      {item.status === "REJECTED" && item.rejectionReason ? (
                        <Text style={rh.rowReason}>Reason: {item.rejectionReason}</Text>
                      ) : null}
                    </View>
                    <View style={[rh.pill, { backgroundColor: `${statusColor}15` }]}>
                      <Text style={[rh.pillTxt, { color: statusColor }]}>{item.status}</Text>
                    </View>
                  </View>
                );
              })}
...
const rh = StyleSheet.create({
  card: { backgroundColor: T.paper, borderRadius: T.radiusLg, padding: 16, borderWidth: 1, borderColor: T.border, gap: 10 },
  header: { flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 4 },
  headerTxt: { fontSize: 13, fontWeight: "700", color: T.textPrimary },
  row: { flexDirection: "row", alignItems: "center", gap: 10 },
  statusDot: { width: 28, height: 28, borderRadius: 8, alignItems: "center", justifyContent: "center" },
  rowTitle: { fontSize: 13, fontWeight: "600", color: T.textPrimary },
  rowSub: { fontSize: 12, color: T.textSecondary, fontWeight: "400" },
  rowReason: { fontSize: 11, color: T.error, marginTop: 2 },
  pill: { borderRadius: T.radiusFull, paddingHorizontal: 8, paddingVertical: 3 },
  pillTxt: { fontSize: 10, fontWeight: "700", textTransform: "uppercase" },
});
