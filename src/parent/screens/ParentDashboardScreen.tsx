/**
 * ParentDashboardScreen.tsx â€” YourShikshak
 * Mirrors TutorDashboardScreen exactly: same layout, same colours,
 * same card shapes, same strip/KPI/section-head patterns.
 *
 * Two states:
 *   1. Empty  â€” no active class: how-it-works + "Request a Tutor" CTA
 *   2. Active â€” KPI strip + card grid + upcoming sessions + teacher card + activity feed
 */
...
// â”€â”€â”€ Empty tab state (bouncing emoji) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ Request Tutor bottom-sheet â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    >
      <View style={am.overlay}>
        ...
        <View style={[am.sheet, { marginBottom: kbHeight }]}>
          ...
          <TextInput
            style={am.input}
            placeholder="e.g. Mathematics, Science, Englishâ€¦"
          ...
          <TextInput
            style={am.input}
            placeholder="e.g. Class 8, Grade 10â€¦"
            placeholderTextColor={T.textDisabled}
            value={grade}
...
// â”€â”€â”€ Concern bottom-sheet â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

const CONCERN_TYPES = [
  { key: "CONCERN",   label: "General Concern" },
...
const ConcernSheet = ({
  ...
  visible: boolean;
  onClose: () => void;
  onSubmit: (subject: string, msg: string, type: string) => void;
  submitting: boolean;
}) => {
  const [subject, setSubject] = useState("");
  ...
  useEffect(() => {
    if (!visible) { setSubject(""); setMsg(""); setType("CONCERN"); }
  }, [visible]);
  ...
  const canSubmit = subject.trim().length > 0 && msg.trim().length > 0 && !submitting;
        ...
        <View style={[am.sheet, { marginBottom: kbHeight }]}>
          ...
          <View style={am.header}>
            <View style={[am.headerIcon, { backgroundColor: `${T.error}12` }]}>
              <Ionicons name="ticket-outline" size={20} color={T.error} />
            </View>
            <View style={{ flex: 1 }}>
              <Text style={am.headerTitle}>Raise a Ticket</Text>
              <Text style={am.headerSub}>A ticket will be created and tracked until resolved</Text>
            </View>
            <Pressable onPress={onClose} hitSlop={12}>
              <Ionicons name="close" size={20} color={T.mutedFg} />
            </Pressable>
          ...
          {/* Type selector */}
          <Text style={am.fieldLabel}>Type</Text>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 12 }}>
            <View style={{ flexDirection: "row", gap: 6 }}>
              {CONCERN_TYPES.map((t) => (
                <Pressable
                  key={t.key}
                  style={[am.typeChip, type === t.key && am.typeChipActive]}
                  onPress={() => setType(t.key)}
                >
                  <Text style={[am.typeChipTxt, type === t.key && am.typeChipTxtActive]}>{t.label}</Text>
                </Pressable>
              ))}
            </View>
          </ScrollView>
          ...
          <Text style={am.fieldLabel}>Description *</Text>
          <TextInput
            style={[am.input, { minHeight: 80, textAlignVertical: "top" }]}
            placeholder="e.g. Tutor was late, topic not coveredâ€¦"
            placeholderTextColor={T.textDisabled}
            value={msg}
            onChangeText={setMsg}
            multiline
          ...
          <Pressable
            style={[am.submitBtn, { backgroundColor: T.error }, !canSubmit && { opacity: 0.5 }]}
            onPress={() => onSubmit(subject, msg, type)}
            disabled={!canSubmit}
          >
            {submitting ? (
              <ActivityIndicator color="#fff" size="small" />
              ...
              <>
                ...
                <Text style={am.submitTxt}>Submit Ticket</Text>
              </>
            )}
          </Pressable>
        </View>
...
// â”€â”€â”€ Pay Fees Sheet â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ Reschedule Sheet â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ Sidebar (drawer) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      ...
      <View style={{ flex: 1 }}>
        {onChildProfile && (
          <Pressable
            ...
            onPress={onChildProfile}
          >
            <View style={[sb.navIconBg, { backgroundColor: `${T.success}18` }]}>
              <Ionicons name="person-outline" size={18} color={T.success} />
            </View>
            <Text style={sb.navLabel}>Child Profile</Text>
        ...
        {onSettings && (
          ...
          >
            <View style={[sb.navIconBg, { backgroundColor: `${T.primary}18` }]}>
              <Ionicons name="settings-outline" size={18} color={T.primary} />
            </View>
...
// â”€â”€â”€ Pending Request Card â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
const PendingRequestCard = ({
  ...
  const STEP_META = [
    ...
    },
    {
      ...
      desc: "Almost there â€” confirm to begin classes",
    },
  ];
    ...
    <View style={prc.wrap}>
      ...
      >
        ...
        <Text style={prc.subject} numberOfLines={1}>
          ...
          {pendingRequest.grade ? `  Â·  Grade ${pendingRequest.grade}` : ""}
        </Text>
...
const ParentDashboardScreen = () => {
  ...
  const handleConcernSubmit = async (subject: string, msg: string, type: string) => {
    if (!data?.activeClass) {
      showError("No Active Class", "You don't have an active class to raise a concern for. Please contact support.");
      return;
    }
    setSubmitting(true);
    try {
      const res = await createParentTicket({
        subject,
        description: msg,
        type: type as any,
      ...
      showSuccess("Ticket Created", `Your ticket ${res.data.ticketNumber} has been raised. We'll follow up shortly.`);
    } catch (e: any) {
      showError("Failed", e?.message || "Could not raise ticket.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  // â”€â”€ Strip values â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const attPct =
  ...
  return (
    <View style={s.root}>
      ...
      >
        ...
        >
          ...
          <View style={s.greetBlock}>
            ...
            {!loading && hasActiveClass && cls?.studentName ? (
              timeUntilNext ? (
                <Text style={s.greetContext}>
                  {cls.studentName} has class in {timeUntilNext}
                </Text>
              ) : cls?.nextSessionDate ? (
                <Text style={s.greetContext}>
                  No class today Â· Next: {fmtDate(cls.nextSessionDate)}
                </Text>
              ) : null
            ) : null}
        ...
        <View style={s.card}>
          {loading ? (
            ...
            /* â”€â”€ Active KPI view â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
            <>
              {/* â”€â”€ Progress Pulse Card (emotional hero) â”€â”€ */}
              {data?.latestTest ? (() => {
                const pct = Math.round((data.latestTest.score / data.latestTest.totalMarks) * 100);
                const isGood = pct >= 75;
                const isDecline = pct < 50;
                const trendColor = isDecline ? T.error : isGood ? T.success : T.warning;
                const trendIcon = isDecline ? "trending-down" : isGood ? "trending-up" : "remove-circle-outline";
                const trendLabel = isDecline ? "Needs attention" : isGood ? "Improving" : "Steady";
                const trendArrow = isDecline ? "â†“" : isGood ? "â†‘" : "â†’";
                const gradColors: [string, string] = isDecline
                  ? [`${T.error}18`, `${T.error}08`]
                  : isGood
                  ? [`${T.success}18`, `${T.success}08`]
                  : [`${T.warning}18`, `${T.warning}08`];
                return (
                  <Pressable
                    style={ppc.heroWrap}
                    onPress={() => (navigation as any).navigate?.("ParentProgress")}
                    android_ripple={{ color: `${trendColor}20` }}
                  >
                    <LinearGradient colors={gradColors} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={ppc.heroGrad}>
                      <View style={ppc.heroTop}>
                        <View style={[ppc.heroIconBox, { backgroundColor: `${trendColor}20` }]}>
                          <Ionicons name={trendIcon as any} size={20} color={trendColor} />
                        </View>
                        <View style={{ flex: 1 }}>
                          <Text style={ppc.heroTagline}>Progress Pulse</Text>
                          <Text style={[ppc.heroTrendLabel, { color: trendColor }]}>{trendArrow} {trendLabel}</Text>
                        </View>
                        <View style={[ppc.heroBadge, { backgroundColor: `${trendColor}20`, borderColor: `${trendColor}30` }]}>
                          <Text style={[ppc.heroBadgeTxt, { color: trendColor }]}>{pct}%</Text>
                        </View>
                      </View>
                      <View style={ppc.heroScoreRow}>
                        <Text style={ppc.heroScore}>
                          {data.latestTest.score}
                          <Text style={ppc.heroScoreOf}>/{data.latestTest.totalMarks}</Text>
                        </Text>
                        <Text style={ppc.heroSubject}>{data.latestTest.subject}</Text>
                      </View>
                      <Text style={ppc.heroInsight}>
                        {isGood
                          ? `${cls?.studentName ?? "Your child"} is on track. Sessions are paying off. ðŸŽ¯`
                          : isDecline
                          ? `There's room to grow in ${data.latestTest.subject}. Your coordinator is monitoring closely.`
                          : `${cls?.studentName ?? "Your child"} is holding steady. A push this week could make the difference.`}
                      </Text>
                      <View style={ppc.heroFooter}>
                        <Text style={[ppc.heroFooterTxt, { color: trendColor }]}>View full progress â†’</Text>
                      </View>
                    </LinearGradient>
                  </Pressable>
                );
              ...
              {/* â”€â”€ Contextual Banner (one at a time, highest priority) â”€â”€ */}
              {(() => {
                // Priority 1: fees due â‰¤3 days
                const feesUrgent = paymentSummary?.nextPayment &&
                  paymentSummary.nextPayment.status !== "PAID" &&
                  (() => {
                    const d = new Date(paymentSummary!.nextPayment!.dueDate);
                    const diff = Math.ceil((d.getTime() - Date.now()) / 86_400_000);
                    return diff >= 0 && diff <= 3 ? diff : null;
                  })();
                if (feesUrgent !== null && feesUrgent !== undefined && feesUrgent !== false) {
                  return (
                    <Pressable style={[cb.card, { borderColor: `${T.warning}40`, backgroundColor: `${T.warning}10` }]} onPress={() => setShowPaySheet(true)}>
                      <Ionicons name="wallet-outline" size={16} color={T.warning} />
                      <View style={{ flex: 1 }}>
                        <Text style={cb.title}>Fee payment due soon</Text>
                        <Text style={cb.sub}>
                          {feesUrgent === 0
                            ? `Pay today to keep ${cls?.studentName ?? "your child"}'s classes uninterrupted.`
                            : `Pay by ${new Date(paymentSummary!.nextPayment!.dueDate).toLocaleDateString("en-IN", { day: "numeric", month: "short" })} to keep classes uninterrupted.`}
                        </Text>
                      </View>
                ...
                // Priority 2: attendance low
                if (cls?.attendancePercentage != null && cls.attendancePercentage < 75) {
                  return (
                    <View style={[cb.card, { borderColor: `${T.warning}40`, backgroundColor: `${T.warning}10` }]}>
                      ...
                      <View style={tc.card}>
                        ...
                        <View style={tc.topRow}>
                          ...
                          <View style={tc.modePill}>
                            ...
                            />
                            <Text style={[tc.modeTxt, { color: cls.mode === "ONLINE" ? T.primary : T.secondary }]}>
                              {cls.mode}
                            </Text>
                          </View>
                        </View>
                        ...
                        <View style={tc.actionRow}>
                          <Pressable
                            ...
                            onPress={() => setShowConcern(true)}
                          >
                            <Ionicons name="chatbubble-outline" size={13} color={T.primary} />
                            <Text style={tc.actionTxt}>Message Coordinator</Text>
                          </Pressable>
                          <Pressable
                            style={[tc.actionBtn, tc.actionBtnSecondary]}
                            onPress={() => navigation.navigate("TutorProfile", { viewerRole: "PARENT" })}
                          >
                            <Ionicons name="person-outline" size={13} color={T.textSecondary} />
                            <Text style={[tc.actionTxt, { color: T.textSecondary }]}>View Profile</Text>
                          </Pressable>
                        </View>
                      </View>
                    </>
                  ...
                  {(data?.upcomingSessions?.length ?? 0) === 0 ? (
                    ...
                    data!.upcomingSessions!.map((sess) => (
                      <View key={sess._id} style={up.card}>
                        ...
                        <View style={{ flex: 1 }}>
                          <Text style={up.subject}>{cls?.subject ?? "â€”"}</Text>
                          <Text style={up.time}>
                            {sess.timeSlot} Â· Session {sess.sessionNumber}
                          </Text>
                        </View>
                        <View
                          style={[
      ...
      {/* Sidebar removed â€” navigation moved to tab bar More sheet */}
...
// Progress Pulse Card (hero)
const ppc = StyleSheet.create({
  heroWrap: { borderRadius: T.radiusLg, overflow: "hidden", marginBottom: 4 },
  heroGrad: { padding: 18, paddingBottom: 14 },
  heroTop: { flexDirection: "row", alignItems: "center", marginBottom: 14 },
  heroIconBox: { width: 32, height: 32, borderRadius: 10, backgroundColor: "rgba(255,255,255,0.2)", alignItems: "center", justifyContent: "center", marginRight: 10 },
  heroTagline: { flex: 1, fontSize: 11, fontWeight: "700", color: "rgba(255,255,255,0.85)", textTransform: "uppercase", letterSpacing: 0.6 },
  heroTrendLabel: { fontSize: 11, fontWeight: "600", color: "rgba(255,255,255,0.85)" },
  heroBadge: { backgroundColor: "rgba(255,255,255,0.2)", borderRadius: T.radiusFull, paddingHorizontal: 9, paddingVertical: 3, marginLeft: 6 },
  heroBadgeTxt: { fontSize: 10, fontWeight: "800", color: "#fff" },
  heroScoreRow: { flexDirection: "row", alignItems: "flex-end", gap: 6, marginBottom: 8 },
  heroScore: { fontSize: 44, fontWeight: "800", color: "#fff", lineHeight: 48 },
  heroScoreOf: { fontSize: 20, fontWeight: "600", color: "rgba(255,255,255,0.6)", paddingBottom: 4 },
  heroSubject: { flex: 1, fontSize: 13, fontWeight: "600", color: "rgba(255,255,255,0.8)", paddingBottom: 6 },
  heroInsight: { fontSize: 12, color: "rgba(255,255,255,0.75)", lineHeight: 18, fontStyle: "italic", marginBottom: 14 },
  heroFooter: { borderTopWidth: 1, borderTopColor: "rgba(255,255,255,0.15)", paddingTop: 10, flexDirection: "row", alignItems: "center", justifyContent: "flex-end", gap: 4 },
  heroFooterTxt: { fontSize: 12, fontWeight: "600", color: "rgba(255,255,255,0.9)" },
});
...
const tc = StyleSheet.create({
  ...
  actionTxt: { fontSize: 11, fontWeight: "600", color: T.primary },
});
...
// Bottom sheet modal (shared) â€” mirrors TutorDashboardScreen am styles exactly
const am = StyleSheet.create({
  ...
  typeChip: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: T.radiusFull, backgroundColor: T.muted, borderWidth: 1, borderColor: T.border },
  typeChipActive: { backgroundColor: T.primary, borderColor: T.primary },
  typeChipTxt: { fontSize: 12, fontWeight: "600", color: T.mutedFg },
