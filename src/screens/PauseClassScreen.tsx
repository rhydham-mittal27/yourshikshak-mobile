    ...
    <View style={pc.calOuter}>
      ...
      <View style={pc.calWrap}>
        <View style={pc.calHeader}>
          <TouchableOpacity onPress={() => setViewMonth(new Date(year, month - 1, 1))} style={pc.calNav}>
            <Ionicons name="chevron-back" size={15} color={T.textPrimary} />
          </TouchableOpacity>
          <Text style={pc.calTitle}>{MONTH_NAMES[month]} {year}</Text>
          <TouchableOpacity onPress={() => setViewMonth(new Date(year, month + 1, 1))} style={pc.calNav}>
            <Ionicons name="chevron-forward" size={15} color={T.textPrimary} />
          </TouchableOpacity>
        </View>
        <View style={pc.calDayRow}>
          {DAYS_SHORT.map((d) => <Text key={d} style={pc.calDayLabel}>{d}</Text>)}
        </View>
        <View style={pc.calGrid}>
          {cells.map((day, idx) => {
            ...
            return (
              <TouchableOpacity key={day} style={pc.calCell} onPress={() => !isPast && onSelect(date)} disabled={isPast}>
                <View style={[pc.calDayWrap, isSel && { backgroundColor: T.primary }]}>
                  <Text style={[pc.calDayNum, isPast && { color: T.textDisabled }, isSel && { color: "#fff" }]}>{day}</Text>
                </View>
              </TouchableOpacity>
            );
          })}
      ...
      {selected && (
        <View style={pc.selPill}>
          ...
          <Text style={pc.selPillTxt}>{selected.toLocaleDateString("en-IN", { weekday: "short", day: "numeric", month: "short" })}</Text>
        </View>
      )}
    </View>
  );
...
const PauseClassScreen = () => {
  ...
  return (
    <View style={pc.root}>
      ...
      {submitted ? (
        <Animated.View style={[pc.successWrap, { opacity: successAnim, transform: [{ scale: successAnim.interpolate({ inputRange: [0, 1], outputRange: [0.95, 1] }) }] }]}>
          ...
          <Text style={pc.successSub}>
            Your class has been paused{"\n"}from <Text style={{ fontWeight: "800" }}>{fmt(startDate!)}</Text>
            {untilFurtherNotice ? " until further notice" : ` to ${fmt(endDate!)}`}.
          </Text>
          <View style={pc.noteCard}>
            <Ionicons name="information-circle-outline" size={15} color={T.info} />
            <Text style={pc.noteTxt}>
              Billing is adjusted for the pause period. You'll receive a confirmation from our team.
            </Text>
          </View>
        ...
        <ScrollView
          style={{ flex: 1 }}
          contentContainerStyle={[pc.content, { paddingBottom: insets.bottom + 100 }]}
        ...
        >
          ...
          <View style={pc.billingNote}>
            ...
            <Text style={pc.billingTxt}>
              Classes during the pause period will not be billed. Billing resumes automatically when classes resume.
            </Text>
          </View>
          ...
          <View style={pc.section}>
            ...
            <View style={pc.reasonGrid}>
              {PAUSE_REASONS.map((r) => (
                <TouchableOpacity
                  key={r.id}
                  style={[pc.reasonCard, reason === r.id && pc.reasonCardActive]}
                  onPress={() => setReason(r.id)}
                >
                  <View style={[pc.reasonIcon, reason === r.id && { backgroundColor: `${T.warning}20` }]}>
                    <Ionicons name={r.icon as any} size={18} color={reason === r.id ? T.warning : T.textSecondary} />
                  </View>
                  <Text style={[pc.reasonLabel, reason === r.id && { color: T.warning, fontWeight: "700" }]}>
                    {r.label}
                  </Text>
                  {reason === r.id && (
          ...
          <View style={pc.section}>
            <View style={pc.toggleRow}>
              <View style={{ flex: 1 }}>
                ...
                <Text style={pc.toggleSub}>Resume only when you request it</Text>
              </View>
              <Switch
          ...
          {startDate && (untilFurtherNotice || endDate) && reason && (
            <View style={pc.summaryCard}>
              ...
              <View style={pc.summaryRow}>
                <Ionicons name="pause-circle-outline" size={14} color={T.warning} />
                <Text style={pc.summaryTxt}>
                  Pause from <Text style={{ fontWeight: "800" }}>{fmt(startDate)}</Text>
                  {untilFurtherNotice ? " until you resume" : ` to ${fmt(endDate!)}`}
                </Text>
              </View>
              <View style={pc.summaryRow}>
                <Ionicons name="information-circle-outline" size={14} color={T.warning} />
                <Text style={pc.summaryTxt}>
                  Reason: <Text style={{ fontWeight: "800" }}>
      ...
      {!submitted && (
        <View style={[pc.footer, { paddingBottom: insets.bottom + 8 }]}>
          <Pressable
            ...
            disabled={!canSubmit}
            style={({ pressed }) => [pc.submitBtn, !canSubmit && pc.submitBtnDisabled, pressed && { opacity: 0.85 }]}
          >
            <LinearGradient
              colors={canSubmit ? [T.warning, "#d97706"] : [T.textDisabled, T.textDisabled]}
...
const pc = StyleSheet.create({
  ...
  orbA:         { position: "absolute", width: 180, height: 180, borderRadius: 90, backgroundColor: "rgba(251,191,36,0.08)", top: -50, right: -40 } as any,
  ...
  billingNote:  { flexDirection: "row", alignItems: "flex-start", gap: 8, backgroundColor: `${T.info}08`, borderRadius: T.radiusMd, padding: 12, borderWidth: 1, borderColor: `${T.info}18` },
  billingTxt:   { flex: 1, fontSize: 12, color: T.info, fontWeight: "600", lineHeight: 18 },
  ...
  reasonCard:   { width: "47%", flexDirection: "row", alignItems: "center", gap: 8, padding: 10, backgroundColor: T.muted, borderRadius: T.radiusMd, borderWidth: 1, borderColor: T.border, position: "relative" },
  reasonCardActive: { backgroundColor: `${T.warning}08`, borderColor: T.warning },
  reasonIcon:   { width: 34, height: 34, borderRadius: 17, backgroundColor: T.muted, alignItems: "center", justifyContent: "center" },
  reasonLabel:  { flex: 1, fontSize: 11, color: T.textSecondary, fontWeight: "600", lineHeight: 15 },
  reasonCheck:  { position: "absolute", top: -4, right: -4, width: 16, height: 16, borderRadius: 8, backgroundColor: T.warning, alignItems: "center", justifyContent: "center" } as any,

  calOuter:     { gap: 8 },
  calLabel:     { fontSize: 13, fontWeight: "800", color: T.textDisabled, letterSpacing: 0.6, textTransform: "uppercase", paddingHorizontal: 2 },
  calWrap:      { backgroundColor: T.paper, borderRadius: T.radiusXl, padding: T.base, borderWidth: 1, borderColor: T.border, gap: 8 },
  calHeader:    { flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
  calNav:       { width: 28, height: 28, borderRadius: 14, alignItems: "center", justifyContent: "center", backgroundColor: T.muted },
  ...
  selPill:      { flexDirection: "row", alignItems: "center", gap: 6, backgroundColor: `${T.primary}10`, borderRadius: 99, paddingHorizontal: 12, paddingVertical: 5, alignSelf: "flex-start" },
  ...
  summaryCard:  { backgroundColor: `${T.warning}08`, borderRadius: T.radiusXl, padding: T.base, borderWidth: 1, borderColor: `${T.warning}25`, gap: 8 },
  summaryHead:  { fontSize: 12, fontWeight: "800", color: T.warning, letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 4 },
  summaryRow:   { flexDirection: "row", alignItems: "flex-start", gap: 8 },
  summaryTxt:   { fontSize: 13, color: T.textSecondary, fontWeight: "500", flex: 1, lineHeight: 19 },
  ...
  successTitle: { fontSize: 22, fontWeight: "800", color: T.textPrimary, marginBottom: 8, marginTop: 12, letterSpacing: -0.4 },
  successSub:   { fontSize: 14, color: T.textSecondary, textAlign: "center", lineHeight: 22, marginBottom: T.lg },
  noteCard:     { flexDirection: "row", alignItems: "flex-start", gap: 8, backgroundColor: `${T.info}08`, borderRadius: T.radiusMd, padding: 12, borderWidth: 1, borderColor: `${T.info}18`, marginBottom: T.xl },
  noteTxt:      { flex: 1, fontSize: 12, color: T.info, fontWeight: "600", lineHeight: 18 },
  doneBtn:      { backgroundColor: T.warning, borderRadius: T.radiusFull, paddingHorizontal: 36, paddingVertical: 13 },
